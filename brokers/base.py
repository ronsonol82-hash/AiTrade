...
class OrderResult:
    ...
    create_time: Optional[datetime] = None  # время создания ордера (если доступно у брокера)
...
class BrokerAPI(ABC):
    ...
    async def get_order_info(
        ...
        *,
        ...
        """Опционально: получить сырой статус ордера по order_id/client_id."""
        raise NotImplementedError(f"{self.__class__.__name__}.get_order_info not implemented")
    ...
    async def wait_for_order_final(
        ...
        *,
        ...
        """Опционально: дождаться финального статуса (filled/cancelled/rejected)."""
        raise NotImplementedError(f"{self.__class__.__name__}.wait_for_order_final not implemented")

    async def list_active_orders(self, symbol: str | None = None) -> List[OrderResult]:
        """Опционально: список активных ордеров (для kill-switch)."""
        raise NotImplementedError(f"{self.__class__.__name__}.list_active_orders not implemented")
