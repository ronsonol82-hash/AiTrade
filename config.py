...
class Config:
    ...
    PROTECTIONS_FILE = os.path.join(STATE_DIR, "protections.json")
    RUNNER_STATE_FILE = os.path.join(STATE_DIR, "runner_state.json")
    SNAPSHOT_FILE     = os.path.join(STATE_DIR, "last_snapshot.json")
    TRADE_DB_FILE     = os.path.join(STATE_DIR, "trades.sqlite")
    KILL_SWITCH_FILE = os.path.join(STATE_DIR, "kill_switch.json")
    # проверяем kill-switch чаще, чем polling рынка
    KILL_SWITCH_POLL_SECONDS = int(os.getenv("KILL_SWITCH_POLL_SECONDS", "1"))
    # если подряд много ошибок — включаем защитный стоп (kill-switch)
    RUNNER_MAX_CONSECUTIVE_ERRORS = int(os.getenv("RUNNER_MAX_CONSECUTIVE_ERRORS", "5"))
    # ожидание фактического исполнения ордера (сек)
    ORDER_CONFIRM_TIMEOUT_S = float(os.getenv("ORDER_CONFIRM_TIMEOUT_S", "30"))

    LIVE_POLL_SECONDS = 5
    SIGNALS_RELOAD_SECONDS = 60

    # Идентификатор стратегии (важно для детерминированных trade_id)
    STRATEGY_ID = os.getenv("STRATEGY_ID", "universal_live_v1")

    # Включить нативные защиты на Bitget (Шаг 3)
    BITGET_NATIVE_PROTECTION = os.getenv("BITGET_NATIVE_PROTECTION", "0") == "1"
